...
def Main():
     while (True):
          ...
          num = moedaCorrente[0].get("price")
          opa = "%.8f" % float(valorMoeda)

          valorMoeda25 = 0
          valorMoeda100 = 0

          if str(valorLotSize) == "1":
                valorMoeda25 = round(((balance / valorMoeda) / 100) * 25, 0)
                valorMoeda100 = round((balance / valorMoeda), 0)
          else:
                valorMoeda25 = round(((balance / valorMoeda) / 100) * 25, 2)
                valorMoeda100 = round((balance / valorMoeda), 2)
          ...
          menorValorOrdemVenda = ultimasOrdensVenda[5]
          valorOrdemVenda = menorValorOrdemVenda[0]

          ultimasOrdensCompra = ordensDisponiveis.get("bids")
          maiorValorOrdemCompra = ultimasOrdensCompra[5]
