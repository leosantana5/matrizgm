            ...
            if idperfilacessosistema != "cliente" and \
                ...
                if cpf != "nan":
                    ...
                    if results != []:
                        ...
                        # depois fazer a verificação dos dados que ja estão no banco
                    else:
...
for i in base_pdvs:
    ...
    endereco_completo_pdv = f'{logradouro},' \
                        ...
                        f'cep {cep}'
    if status == "Ativo":
        pdv = dbmatriz["pdv_pdvaction"]
        mq = {"codigo": str(codigo)}
        achei = pdv.find(mq)

        results = list(achei)
        if results != []:
            count_pdvs_jacadastrados += 1
            # depois fazer a verificação dos dados que ja estão no banco
            print(f'{count_pdvs_jacadastrados} - Ja Cadastrado - {nome}')
        else:
            adicionar_pdv = {
                "codigo": codigo,
                "nome": nome,
                ...
                "cidade": cidade,
                "uf": uf,
                "latitude": get_lat(endereco_completo_pdv),
                "longitude": get_lng(endereco_completo_pdv),
                "regional": regiao(uf),
                "zona": zona,
                "status": status,
                "endereco_completo":consultar_endereco(endereco_completo_pdv),
                "data_inclusao": str(datetime.timestamp(datetime.now())),
                "data_alteracao": ""
            }
            pdv.insert_one(adicionar_pdv)
            count_pdvs_cadastrados += 1
            print(f'{count_pdvs_cadastrados} - PDV Cadastrado - {nome}')
